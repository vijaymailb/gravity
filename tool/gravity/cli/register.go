...
func RegisterCommands(app *kingpin.Application) *Application {
	...
	g.JoinExecuteCmd.CmdClause = g.JoinCmd.Command("execute", "Join existing cluster or on-going install operation").Default()
	g.JoinExecuteCmd.PeerAddr = g.JoinExecuteCmd.Arg("peer-addrs", "One or several IP addresses of cluster node to join, as comma-separated values").String()
	g.JoinExecuteCmd.AdvertiseAddr = g.JoinExecuteCmd.Flag("advertise-addr", "IP address to advertise").String()
	g.JoinExecuteCmd.Token = g.JoinExecuteCmd.Flag("token", "Unique install token to authorize this node to join the cluster").String()
	g.JoinExecuteCmd.Role = g.JoinExecuteCmd.Flag("role", "Role of this node, optional").String()
	g.JoinExecuteCmd.DockerDevice = g.JoinExecuteCmd.Flag("docker-device", "Docker device to use").Hidden().String()
	g.JoinExecuteCmd.SystemDevice = g.JoinExecuteCmd.Flag("system-device", "Device to use for system data directory").Hidden().String()
	g.JoinExecuteCmd.ServerAddr = g.JoinExecuteCmd.Flag("server-addr", "Address of the agent server").Hidden().String()
	g.JoinExecuteCmd.Mounts = configure.KeyValParam(g.JoinCmd.Flag("mount", "One or several mounts in form <mount-name>:<path>, e.g. data:/var/lib/data"))
	g.JoinExecuteCmd.CloudProvider = g.JoinExecuteCmd.Flag("cloud-provider", "Cloud provider integration e.g. 'generic', 'aws'. If not set, autodetect environment").String()
	g.JoinExecuteCmd.OperationID = g.JoinExecuteCmd.Flag("operation-id", "ID of the operation that was created via UI").Hidden().String()
	g.JoinExecuteCmd.FromService = g.JoinExecuteCmd.Flag("from-service", "Run in service mode").Hidden().Bool()

	g.JoinResumeCmd.CmdClause = g.JoinCmd.Command("resume", "Resume on-going install operation on a joining node")
